...
@testset "Compare MultiDevice in Flux and PSID - FrequencyChirp" begin
    ...
    for P in get_components(x -> PSY.get_name(x) == "Load_2", StandardLoad, sys_train)
    ...
    @test LinearAlgebra.norm(Ir[2] .* -1 .- surrogate_sol.i_series[1, :], Inf) <= 0.0003
    @test LinearAlgebra.norm(Ii[2] .* -1 .- surrogate_sol.i_series[2, :], Inf) <= 0.0002
